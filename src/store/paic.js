//test
const domain = 'http://peimc-smp-stg.pa18.com';

export const queryEmpInfo = domain+'/peimc_smp/rest/loveLetter/queryEmpInfo';
export const sendLoveLetter = domain+'/peimc_smp/rest/loveLetter/sendLoveLetter';
export const recordStatus = domain+'/peimc_smp/rest/loveLetter/recordStatus';
export const uploadPv = domain+'/peimc_smp/rest/tracking/upload';
export const shareUrl = domain+'/peimcnl/tanabata/share.png';